...
class BufferRulesList extends AbstractRulesList
{
    ...
    {
        $minDays = min(array_map(function ($rule) { return $rule->getBufferDays(); }, $rules));

        return new BufferDaysRule(array(
            'buffer_days'   => $minDays,
            'season_ids'    => array(0),
    ...
    public static function create($rules)
    {
        ...
        $bufferDays = array_map(function ($rule) { return new BufferDaysRule($rule); }, $rules);
