...
class ReservationRulesList extends AbstractRulesList
{
    ...
    {
        switch ($this->type) {
            case ReservationRules::RULE_CHECK_IN   : return $this->combineCheckInRules($rules);    break;
            case ReservationRules::RULE_CHECK_OUT  : return $this->combineCheckOutRules($rules);   break;
            case ReservationRules::RULE_MIN_STAY   : return $this->combineMinStayRules($rules);    break;
            case ReservationRules::RULE_MAX_STAY   : return $this->combineMaxStayRules($rules);    break;
            case ReservationRules::RULE_MIN_ADVANCE: return $this->combineMinAdvanceRules($rules); break;
            case ReservationRules::RULE_MAX_ADVANCE: return $this->combineMaxAdvanceRules($rules); break;
        }
    ...
    {
        $minStay = min(array_map(function ($rule) { return $rule->getMinDays(); }, $rules));
    ...
    {
        $maxStay = max(array_map(function ($rule) { return $rule->getMaxDays(); }, $rules));
    ...
    {
        $minAdvance = min(array_map(function ($rule) { return $rule->getMinAdvanceDays(); }, $rules));

        return new MinAdvanceDaysRule(array(
     ...
     * @since 3.9
     */
    protected function combineMaxAdvanceRules($rules)
    {
        $maxAdvance = max(array_map(function ($rule) { $rule->getMaxAdvanceDays(); }, $rules));

        return new MaxAdvanceDaysRule(array(
