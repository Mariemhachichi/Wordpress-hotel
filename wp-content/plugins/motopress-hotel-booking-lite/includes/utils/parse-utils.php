...
class ParseUtils
{
    ...
    public static function parseCheckInDate($rawData, $args = array())
    {
        ...
        // Parse date
        $checkInString = sanitize_text_field($rawData);
        ...
        } else if (!$allowPastDates && DateUtils::calcNights($today, $checkInDate) < 0) {
    ...
    public static function parseCheckOutDate($rawData, $args = array())
    {
        ...
        // Parse date
        $checkOutString = sanitize_text_field($rawData);
        ...
        } else if ($checkBookingRules && $checkInDate && !MPHB()->getRulesChecker()->verify($checkInDate, $checkOutDate)) {
            throw new Error(__('Nothing found. Please try again with different search parameters.', 'motopress-hotel-booking'));
        } else if ($checkInDate && DateUtils::calcNights($checkInDate, $checkOutDate) < 0) {
    ...
    public static function parseAdults($rawData, $args = array())
    {
        ...
        } else if ($adults == -1 || ($adults >= $minAdults && $adults <= $maxAdults)) {
            return $adults;
        } else {
            throw new Error(__('Adults number is not valid.', 'motopress-hotel-booking'));
        }
    ...
    public static function parseChildren($rawData, $args = array())
    {
        ...
        } else if ($children == -1 || ($children >= $minChildren && $children <= $maxChildren)) {
            return $children;
        } else {
            throw new Error(__('Children number is not valid.', 'motopress-hotel-booking'));
        }
    ...
    public static function parseRooms($rawData, $args)
    {
        ...
        foreach ($roomsByType as $roomTypeId => $roomIds) {
            if (!MPHB()->getRoomPersistence()->isRoomsFree($args['check_in_date'], $args['check_out_date'], $roomIds, array('room_type_id' => $roomTypeId, 'exclude_bookings' => $args['exclude_bookings']))) {
    ...
    protected static function parseRoom($roomData, $args)
    {
        ...
        $allowedRates = MPHB()->getRateRepository()->findAllActiveByRoomType($roomTypeId, array(
            'check_in_date'  => $args['check_in_date'],
            'check_out_date' => $args['check_out_date'],
            'mphb_language'  => 'original'
        ));

        $rateIds = array_map(function ($rate) {
            return $rate->getId();
        }, $allowedRates);
