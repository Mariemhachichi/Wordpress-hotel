...
function courtyard_customize_register( $wp_customize ) {
     ...
     * @access public
     */
    class Courtyard_Upsell_Section extends WP_Customize_Section {
    ...
    $wp_customize->register_section_type( 'Courtyard_Upsell_Section' );

    // Upsell section.
    $wp_customize->add_section(
        new Courtyard_Upsell_Section( $wp_customize, 'custom_theme_upsell',
            array(
                'title'    => esc_html__( 'Need More Options?', 'courtyard' ),
                'pro_text' => esc_html__( 'Buy PRO Version', 'courtyard' ),
                'pro_url'  => esc_url( 'https://precisethemes.com/wordpress-theme/courtyard-pro/' ),
                'priority' => 1,
            )
        )
    );
