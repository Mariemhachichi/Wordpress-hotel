<?php
/**
 * Di Restaurant Page Metabox options.
 *
 * @package Di Restaurant
 */

/**
 * Class Di_Restaurant_Page_Metabox.
 */
class Di_Restaurant_Page_Metabox {
	...
	public function add_meta_box( $post_type ) {
		if( $post_type == 'page' ) {
			add_meta_box(
				'di_restaurant_page_meta_box_cntnr',
				__( 'Di Restaurant Theme Options for this Page', 'di-restaurant' ),
	...
	public function render_meta_box_content( $post ) {
		...
		wp_nonce_field( 'di_restaurant_page_meta_bx_key', 'di_restaurant_page_meta_bx_key_nonce' );
		...
		<?php
		do_action( 'di_restaurant_page_metabox_render', $post );
	}
	...
	public function save( $post_id ) {
		...
		if( ! isset( $_POST['di_restaurant_page_meta_bx_key_nonce'] ) ) {
			return $post_id;
		}
 
		$nonce = wp_unslash( $_POST['di_restaurant_page_meta_bx_key_nonce'] );
 
		// Verify that the nonce is valid.
		if( ! wp_verify_nonce( $nonce, 'di_restaurant_page_meta_bx_key' ) ) {
			return $post_id;
		}
		...
		do_action( 'di_restaurant_page_metabox_save', $post_id );
	}

}
Di_Restaurant_Page_Metabox::get_instance();